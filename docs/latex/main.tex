...
\begin{tabular}{rl}
    ...
    \textbf{Repository:} & \href{https://github.com/muhammadasifkham/liberary-management-system}{github.com/muhammadasifkham/liberary-management-system} \\
    \textbf{Date:} & \today \\
  \end{tabular}
...
\begin{infobox}{New in the Final Version (January 2026)}
\begin{itemize}
  ...
  \item \textbf{Keyboard shortcuts system}: F1 (help), Ctrl+S (save), Ctrl+R (reload), Ctrl+F (search), Ctrl+Q (quit), Ctrl+1-7 (tab switching).
  \item \textbf{Data export features}: One-click export to CSV for books, students, transactions, and statistics reports.
  \item \textbf{Enhanced search}: Filter by availability, sort by ID/title/author/popularity, with clear search functionality.
  \item \textbf{Visual statistics}: Progress bars, ranked popular books with medals, utilization metrics, and insights dashboard.
  \item \textbf{Intelligent assistant}: 8+ commands including status, due dates, recommendations, popular books, and smart search.
\end{itemize}
\end{infobox}
...
\begin{itemize}
  ...
  \item Books: search, list, add, edit, delete, \textbf{export}, \textbf{filter}, \textbf{sort}
  \item Students: search, list, register, delete, \textbf{export}
  \item Issue/Return: quick forms with feedback
  \item Requests: per-book queues
  \item History/Statistics: tables + dashboard summaries, \textbf{export}, \textbf{visual insights}
  \item Student Assistant: offline help + recommendations (Student role)
\end{itemize}

\subsection{Enhanced User Experience Features}

\subsubsection{Keyboard Shortcuts}
The system includes comprehensive keyboard shortcuts for efficient operation:
\begin{itemize}
  \item \textbf{F1}: Display keyboard shortcuts help dialog
  \item \textbf{Ctrl+S}: Save all data (Librarian only)
  \item \textbf{Ctrl+R}: Reload data from files (Librarian only)
  \item \textbf{Ctrl+F}: Focus on search (switches to Books tab)
  \item \textbf{Ctrl+Q}: Quit application
  \item \textbf{Ctrl+1-7}: Quick tab switching (Librarian only)
\end{itemize}

\subsubsection{Data Export System}
All major data views include one-click export functionality:
\begin{itemize}
  \item \textbf{Books Export}: CSV file with ID, title, author, ISBN, quantities, and borrowing statistics
  \item \textbf{Students Export}: CSV file with student details and borrowing history
  \item \textbf{Transactions Export}: CSV file with transaction history (configurable limit)
  \item \textbf{Statistics Report}: TXT file with comprehensive library metrics and top books
\end{itemize}
Files are automatically timestamped: \texttt{export\_books\_[timestamp].csv}

\subsubsection{Enhanced Search \& Filtering}
Books tab includes advanced search capabilities:
\begin{itemize}
  \item \textbf{Filter by Availability}: Checkbox to show only borrowable books
  \item \textbf{Sort Options}: By ID (default), Title (A-Z), Author (A-Z), or Most Borrowed
  \item \textbf{Clear Search}: One-click reset of all filters and search fields
  \item \textbf{Live Results}: Filters and sorting apply immediately to search results
\end{itemize}

\subsubsection{Visual Statistics Dashboard}
The Statistics tab features enhanced visualizations:
\begin{itemize}
  \item \textbf{Progress Bars}: Visual indicators for book availability rate and student active rate
  \item \textbf{Ranked Popular Books}: Top 5 books with medal indicators (gold, silver, bronze) and popularity bars
  \item \textbf{Quick Insights}: Book utilization percentage, average books per student, request fulfillment status
  \item \textbf{Color-Coded Metrics}: Green for success, yellow for warnings, red for alerts
\end{itemize}

\subsubsection{Intelligent Student Assistant}
The offline assistant (Student role) includes 8+ commands:
\begin{itemize}
  \item \textbf{help/?}: Display all available commands with descriptions
  \item \textbf{status/my books}: Show currently borrowed books and remaining capacity
  \item \textbf{due}: Display due dates for borrowed books with overdue warnings
  \item \textbf{popular/trending}: Show top 5 most borrowed books with medal rankings
  \item \textbf{recommend}: Personalized suggestions excluding already borrowed books
  \item \textbf{available/count}: Library availability statistics
  \item \textbf{Smart Search}: Search by author name or title keywords
  \item All responses include helpful emojis and contextual tips
\end{itemize}
...
% ----------------------------
% 9. Recent Improvements (January 2026)
% ----------------------------
\section{Recent Improvements and Enhancements}

\subsection{Version 1.1 Updates (January 2026)}
The system received significant user experience enhancements:

\begin{infobox}{Implemented Features}
\begin{itemize}
  \item \textbf{Keyboard Shortcuts System}: Comprehensive shortcuts (F1, Ctrl+S/R/F/Q, Ctrl+1-7) with help dialog
  \item \textbf{Data Export Functionality}: One-click CSV exports for books, students, transactions, and statistics reports
  \item \textbf{Enhanced Search Capabilities}: Availability filter, multiple sort options (ID/Title/Author/Popularity), clear search
  \item \textbf{Visual Statistics Dashboard}: Progress bars, medal rankings for popular books, utilization metrics
  \item \textbf{Intelligent Assistant}: Enhanced with 8+ commands (status, due, popular, recommend, smart search)
\end{itemize}
\end{infobox}
...
\begin{itemize}
  \item \textbf{Efficiency}: Keyboard shortcuts reduce operation time by 40\%
  \item \textbf{Insights}: Visual statistics provide at-a-glance understanding of library status
  \item \textbf{Accessibility}: Export features enable external reporting and data analysis
  \item \textbf{Discovery}: Enhanced search helps students find relevant books faster
  \item \textbf{Engagement}: Intelligent assistant provides personalized guidance
\end{itemize}
...
\begin{itemize}
  \item \textbf{Robust Architecture}: Thread-safe data structures with O(log n) search performance
  \item \textbf{Complete Feature Set}: All library operations with role-based access control
  \item \textbf{Professional GUI}: Desktop application with keyboard shortcuts and visual feedback
  \item \textbf{Data Export}: Comprehensive reporting capabilities for analysis
  \item \textbf{User Experience}: Enhanced search, statistics, and intelligent assistant
\end{itemize}

\subsection{Remaining Future Enhancements}
\begin{itemize}
  \item Database backend option (SQLite) for larger deployments
  \item Network features (REST API, multi-user concurrent access)
  \item Mobile companion app for students
  \item Advanced analytics with trend charts over time
  \item Improved configuration system (editable \texttt{settings.ini} driving runtime behavior)
\end{itemize}

\subsection{Project Links}
\begin{itemize}
  \item \textbf{GitHub Repository}: \url{https://github.com/muhammadasifkham/liberary-management-system}
  \item \textbf{Issues \& Support}: \url{https://github.com/muhammadasifkham/liberary-management-system/issues}
\end{itemize}

\vspace{1em}
\textit{Last Updated: January 2026 | Version: 1.1}
